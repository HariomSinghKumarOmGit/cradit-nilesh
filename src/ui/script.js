...
let qrInstance = null;
let serverBaseUrl = window.location.origin; // Fallback to current host

function updateQRCode(code) {
  const scanUrl = serverBaseUrl + "/?room=" + code;
...
// Fetch the real network IP from the server so QR codes work on any network
async function fetchServerIP() {
  try {
    ...
    // Running on Vercel or static host — use window.location.origin
    console.log("Using browser origin for QR:", serverBaseUrl);
  }
}

document.addEventListener("DOMContentLoaded", async () => {
  // Fetch real network IP before generating QR code
  await fetchServerIP();
