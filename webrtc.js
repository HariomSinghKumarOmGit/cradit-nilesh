...
// SMART COMPRESSION THRESHOLD: Files smaller than this are sent raw for maximum speed.
// Compression overhead (CPU time for Huffman tree + RLE) isn't worth it for small payloads.
const COMPRESSION_SIZE_THRESHOLD = 1 * 1024 * 1024; // 1 MB
...
async function startSendingChunks() {
  ...
  // SMART FILTER: Fastest route to transfer.
  // Rule 1: Small files (< 1MB) → always skip compression. The CPU cost isn't worth it.
  // Rule 2: Dense binary formats → skip compression. They don't compress well.
  const isSmallFile = file.size < COMPRESSION_SIZE_THRESHOLD;
  const isDenseBinary =
    ...
    file.type === "";
  const skipCompression = isSmallFile || isDenseBinary;
  ...
  if (skipCompression) {
    const reason = isSmallFile ? `small file (${fmtBytes(file.size)} < 1 MB)` : "dense binary format";
    $("prog-sub").textContent = `⚡ Direct send — ${reason}`;
    console.log(
      `⚡ Smart Filter: Raw transfer (${reason}) @ ${netChunkSize / 1024}KB chunks`,
    );
  } else {
